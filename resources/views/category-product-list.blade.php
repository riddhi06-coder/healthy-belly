...
?>
                ...
                @endforeach
                    ...
                    <?php if(isset($variant_prod_id)){ ?>
                    <button class="default-btn1 product_price-{{$key}} add_product" data-id="{{$all_products->id}}" data-variant="{{$variant_prod_id->id}}">Add to cart</button>
                    <?php } ?>
                    </div>
                    <?php }else{ ?>
    ...
    function filter_data()
    {
      ...
      $.ajax({
          url:"{{route('category_filter_product_data')}}",
          type: "POST",
          data:{'action':action,
