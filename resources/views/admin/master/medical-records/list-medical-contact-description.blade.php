...
<div class="page-wrapper">
					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">List Medical Records Description</li></a>
							</ol>
						</nav>
					</div>
				</div>
							...
							<table id="example2" class="table table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
                                        ...
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_medical_contact_description1(this)" checked>
                                        </div>
                                        </td>
                                        @else
                                        <td>
                                        <div class="form-check form-switch form-check-warning">
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_medical_contact_description1(this)">
                                        </div>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="{{route('medical_records.edit_medical_contact_description')}}/{{$value->id}}" class="btn btn-info">edit</a>
		...
		<script>
function status_check_medical_contact_description1(checkbox) {
    ...
    $.ajax({
        url: '{{ route('medical_records.status_change_medical_contact_description') }}',
        type: 'POST',
        headers: {
