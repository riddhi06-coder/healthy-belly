...
<div class="page-wrapper">
					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">List About Medical Records</li></a>
							</ol>
						</nav>
					</div>
				</div>
							...
							<table id="example2" class="table table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
										...
										<td>{{$value->third_title}}</td>
										<td>{!!$value->third_description!!}</td>
                                        @if($value->status == 1)
                                        <td>
                                        <div class="form-check form-switch form-check-warning">
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_about_medical_records(this)" checked>
                                        </div>
                                        </td>
                                        @else
                                        <td>
                                        <div class="form-check form-switch form-check-warning">
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_about_medical_records(this)">
                                        </div>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="{{route('medical_records.edit_about_medical_records')}}/{{$value->id}}" class="btn btn-info">edit</a>
		...
		<script>
function status_check_about_medical_records(checkbox) {
    ...
    $.ajax({
        url: '{{ route('medical_records.status_change_about_medical_records') }}',
        type: 'POST',
        headers: {
