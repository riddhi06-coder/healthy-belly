				  ...
				  <div class="card">
				    ...
				    <div class="card-body">
					  ...
					  <div class="bs-stepper-content">
						<form action="{{route('society.update_society_details')}}" method="post" enctype="multipart/form-data">
							@csrf
                            <input type="hidden" name="id" class="form-control" value="{{$data->id}}" name="id">

						  <div id="test-l-1" role="tabpanel" class="bs-stepper-pane" aria-labelledby="stepper1trigger1">
							<h5 class="mb-1">Add Society Details </h5><br>
							...
							<div class="row g-3">
								...
								<div class="col-12 col-lg-6">
									...
									<input type="text" name="objectives_title" class="form-control" value="{{$data->objectives_title}}" name="objective_title">
								...
								@foreach(explode(',',$data->objectives_description) as $key=>$val)
								<div class="input-row" style="display: flex;">
								<input type="text" style="flex: 1; margin-right: 10px;" class="input form-control" id="InputID{{ $key }}" name="objectives_description[]" value="{{ $val }}" >
								</div>
								@endforeach
								</div>
								@error('objectives_description')
								<div class="error text-danger">
								{{ $errors->first('objectives_description') }}
								</div>
								@enderror
								</div>	
								...
								<div class="col-12 col-lg-6">
									...
									<input type="text" name="institutions_title" class="form-control" value="{{$data->institutions_title}}" name="institutions_title">
                                ...
                                @foreach(explode('[/]',$data->institutions_description) as $key=>$val)
                                <input type="text" style="flex: 1; margin-right: 10px;" class="input1 form-control"  value="{{ $val }}" id="InputID1{{ $key }}" name="institutions_description[]" value="">
                               @endforeach
                            </div>
                                </div>
