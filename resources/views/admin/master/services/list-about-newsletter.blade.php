...
<div class="page-wrapper">
					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">List About Newsletter</li></a>
							</ol>
						</nav>
					</div>
				</div>
							...
							<table id="example2" class="table table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
                                        ...
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_about_newsletter(this)" checked>
                                        </div>
                                        </td>
                                        @else
                                        <td>
                                        <div class="form-check form-switch form-check-warning">
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_about_newsletter(this)">
                                        </div>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="{{route('services.edit_about_newsletter')}}/{{$value->id}}" class="btn btn-info">edit</a>
		...
		<script>
function status_check_about_newsletter(checkbox) {
    ...
    $.ajax({
        url: '{{ route('services.status_change_about_newsletter') }}',
        type: 'POST',
        headers: {
