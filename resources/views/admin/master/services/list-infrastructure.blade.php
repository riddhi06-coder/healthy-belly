...
<div class="page-wrapper">
			<div class="page-content">
				...
				<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
					<div class="breadcrumb-title pe-3">Infrastructure</div>
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">List Library Services Description</li></a>
							</ol>
						</nav>
					</div>
				</div>
							...
							<table id="example2" class="table table-striped table-bordered">
								...
								<tbody>
									...
									<tr>
                                        ...
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_library_service_description(this)" checked>
                                        </div>
                                        </td>
                                        @else
                                        <td>
                                        <div class="form-check form-switch form-check-warning">
                                        <input class="form-check-input" type="checkbox" data-id="{{$value->id}}" onclick="status_check_library_service_description(this)">
                                        </div>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="{{route('services.edit_library_service_description')}}/{{$value->id}}" class="btn btn-info">edit</a>
		...
		<script>
function status_check_library_service_description(checkbox) {
    ...
    $.ajax({
        url: '{{ route('services.status_change_library_service_description') }}',
        type: 'POST',
        headers: {
